...
export function generateLog(type, player1, player2, damage) {
	...
	switch(type) {
		...
		case 'hit':
			randomLogNumber = getRandomNumber(0, hit.length - 1);
			text = hit[randomLogNumber]
							.replace('[playerKick]', namePlayer1)
							.replace('[playerDefence]', namePlayer2);
			log = `${time} - ${text} - ${damage} [${hpPlayer2}/100]`;
			break;

		case 'defence':
			randomLogNumber = getRandomNumber(0, defence.length - 1);
			text = defence[randomLogNumber]
							.replace('[playerKick]', namePlayer1)
							.replace('[playerDefence]', namePlayer2);
			log = `${time} - ${text}`;
			break;
	...
	$chat.insertAdjacentHTML('afterbegin', `<p>${log}</p>`);
}
